...
- **New venue cluster → deep-crawl it.** Whenever a new cluster of venues is imported (a new city, or a neighborhood folded into an existing city), run the deep (monthly-style) crawl for that cluster so it gets deals + events immediately instead of waiting for the 1st-of-month cron: `railway variables --json | node scripts/deep-crawl.js --since <import-date>` (or `--city` / `--neighborhood`). The script self-skips if the monthly crawl is <2 weeks away (it'll cover the cluster soon); `--force` overrides. The deep-crawl and the monthly cron share `crawlVenueSet` in `src/jobs/monthlyCrawl.js`. New cities are also picked up automatically by the monthly cron (it enumerates all cities) and the weekly deal top-up — deep-crawl just front-runs that for freshly-added clusters.
