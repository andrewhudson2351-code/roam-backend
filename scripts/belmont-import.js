// Area-based venue import for Belmont NC — a small town ~12mi west of uptown
// Charlotte, FOLDED INTO the Charlotte city (city='Charlotte') with
// neighborhood='Belmont' so it displays under Charlotte but stays identifiable
// (and so getCityFromCoords on the frontend, which snaps to the nearest CITIES
// entry, already resolves this area to Charlotte). Google Places (New)
// searchNearby over downtown Belmont / Main St. DRY RUN by default; --write
// upserts on google_place_id. Mirrors charleston-import.js.
//
// Env via stdin: railway variables --json | node scripts/belmont-import.js [--write]

const { createClient } = require("@supabase/supabase-js");

const WRITE = process.argv.includes("--write");
const CITY = "Charlotte";           // folded in
const NEIGHBORHOOD = "Belmont";     // all rows labeled Belmont

const AREAS = [
  { label: "Downtown Belmont", lat: 35.2430, lng: -81.0370, radius: 1600 }, // Main St core: String Bean, Nellie's, Sammy's, Old Stone
  { label: "Belmont - Wilkinson/Park", lat: 35.2350, lng: -81.0250, radius: 1800 }, // Primal Brewery, edges toward the river
];
const TYPES = ["bar", "brewery", "pub", "night_club", "restaurant"];
const MUST_APPEAR = ["string bean", "nellie", "sammy", "primal", "old stone"];
const EXCLUDE = /chick-fil-a|mcdonald|taco bell|krispy kreme|chipotle|cook out|quiktrip|starbucks|dunkin|subway|wendy|burger king|popeyes|hardee|arby|panera|jersey mike|jimmy john|food truck|\bcoffee\b|gas station|7-eleven|circle k|sheetz|\bwawa\b|five guys|penn station|meal prep|ben & jerry|ice cream|waffle house|bojangles|zaxby|golden corral|cracker barrel|ihop|denny|culver|domino'?s|noodles and company|steak ?'?n ?shake|harris teeter|food lion|papa john|little caesar|pizza hut|marco'?s pizza|moe'?s southwest|qdoba|tropical smoothie|smoothie king|firehouse subs|\bkfc\b|sonic drive|captain d'?s|long john silver|wingstop|fuel center/i;
const EXCLUDE_ADDR = /$^/;
...
async function main() {
  ...
  console.log(`\nBelmont (into ${CITY}): unique operational ${found.size}, already in DB ${found.size - news.length}, NEW ${news.length}\n`);
  ...
  let wrote = 0;
  for (const n of news) {
    const row = { name: n.name, address: n.address, neighborhood: NEIGHBORHOOD, city: CITY, latitude: n.latitude, longitude: n.longitude, category: n.category, phone: n.place.nationalPhoneNumber || null, website: n.place.websiteUri || null, google_place_id: n.place.id };
